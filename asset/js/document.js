...
$(function(){
  ...
  const headingList = function(){
      ...
      naviHTML += ''
      + '<div id="anker-ht" class="document-title">'
        + '<a class="document-title-link" href="#article">'
          + '<i class="fas fa-book"></i>' + $article.find('h1 > .article-header-title').text()
        + '</a>'
      + '</div>'
      + '<ol class="document-navi-list" data-level="1">';
      $article.find( naviTarget ).each(function(index){
        ...
        // HTML
        if ( nDiff === 1 ) {
          naviHTML += '<ol class="document-navi-list" data-level="' + n + '"';
          if ( n > maxHeadingNum ) {
        ...
        naviHTML += '<li id="anker-' + dataId + '" class="document-navi-item" data-level="' + n + '">'
        + '<a class="document-navi-link" href="#' + id + '">' + t + '</a>';
      });
  ...
  // メニュー開閉
  $('#articleNavi').find('.article-navi-button').on('click', function(){
